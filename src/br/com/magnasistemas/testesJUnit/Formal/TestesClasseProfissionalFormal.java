...
class TestesClasseProfissionalFormal {
	...
	void testeMetodoHorasTrabalhadasMenoresQueQuarenta() {
		...
		Trainee t = new Trainee.Builder().nome("Guilherme").genero(enumGenero.MASCULINO).etnia(enumEtnia.PARDO)
				...
				.diaDeInicio(LocalDate.of(2022, Month.DECEMBER, 05)).pis(23123123).build();
	...
	void testeMetodoTirarFeriasCasoFacaMenosDeUmAnoNaEmpresa() {
		...
		Trainee t = new Trainee.Builder().nome("Guilherme").genero(enumGenero.MASCULINO).etnia(enumEtnia.PARDO)
				...
				.situacaoEscolar(enumSituacaoEscolar.CURSANDO).cargo("Dev").remuneracao(2200.00)
				.carteiraDeTrabalho(23232323).nomeDaEmpresa("MagnaSitemas")
				.diaDeInicio(LocalDate.of(2023, Month.JANUARY, 05)).pis(23123123).build();
	...
	void testaMetodoBaterPonto() {
		...
		int valor = t.baterPonto(LocalTime.of(9, 10), LocalTime.now());
	
		assertEquals(8, t.baterPonto(LocalTime.of(9, 10), LocalTime.now()));
	}
	@Test
	@DisplayName("Esse teste deveria retornar um valor falso caso as horas trabalhadas sejam menores que 40")
	void testaMetodoBaterPontoComHorasIniciaisMaiores() {
		...
		int valor = t.baterPonto(LocalTime.of(18, 10), LocalTime.now());
		
		assertEquals(23, t.baterPonto(LocalTime.of(18, 10), LocalTime.now()));
	}
	
	@Test
	@DisplayName("Esse teste deveria retornar um valor falso caso as horas trabalhadas sejam menores que 40")
	void testaMetodoBaterPontoComHorasIguais() {
		...
		int valor = t.baterPonto(LocalTime.now(), LocalTime.now());
		
		assertEquals(24, t.baterPonto(LocalTime.now(), LocalTime.now()));
	}
