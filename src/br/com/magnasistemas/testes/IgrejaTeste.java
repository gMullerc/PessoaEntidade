...
public class IgrejaTeste {
	public static void main(String[] args) {
		...
		Trainee f = new Trainee.Builder().nome("Guilherme").genero(enumGenero.MASCULINO).etnia(enumEtnia.PARDO)
				.dataDeNascimento(LocalDate.now()).certidaoDeNascimento("324234234234").rg("53123972184")
				.cpf("324234234").contato(1198637)
				.endereco(new Endereco("chinigua", 05730100, 3, "SaoPaulo", "SP", "Brail")).estadoCivil("Solteiro")
		...
		String path = "C:\\Users\\Magna\\eclipse-workspace\\PessoaEntidade\\Igreja.csv";
		...
		try (BufferedReader br = new BufferedReader(new FileReader(path))) {
			...
			while (linhas != null) {
				...
				Igreja i = new Igreja.Builder().nome(nome).genero(genero).etnia(etnia)
		...
		list.get(0).anotarNovoSeguidor(t);
		list.get(0).consultarSeguidores();
		list.get(0).notaFiscalPessoaJuridica("33434334", LocalDate.now());
		list.get(0).notaFiscalPessoaJuridica("33434334", LocalDate.now());
		list.get(0).notaFiscalPessoaJuridica("33434334", LocalDate.now());
		list.get(0).contratarFuncionarioCLT(f);
		list.get(0).ListarFuncionario();
